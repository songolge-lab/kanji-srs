import { CONFIG, saveState, loadState, testStorage, getPersistedSyncCode, persistSyncCode, removePersistedSyncCode, createInitialState, migrateSettings, migrateStats, migrateDecks } from './store/appState.js';
...
incl_subdecks:' (incl. sub-decks)',cancel:'Cancel',no_cards_scope:'No cards in this scope',
...
incl_subdecks:' (alt desteler dahil)',cancel:'İptal',no_cards_scope:'Bu kapsamda kart yok',
...
async function loadApp() {
  ...
  if (saved) { state = saved; state.settings = migrateSettings({ ...CONFIG, ...state.settings }); state.stats = migrateStats(state.stats); state.decks = migrateDecks(state.decks || [], EXAMPLE_DECK_NAMES); }
  ...
  if (existingCode && syncConfigured()) {
    ...
    try { const remote = await cloudPull(existingCode); state = pickNewerState(state, remote ? remote.state : null); state.settings = migrateSettings({ ...CONFIG, ...state.settings }); state.decks = migrateDecks(state.decks || [], EXAMPLE_DECK_NAMES); save(); syncStatus = 'synced'; }
...
document.getElementById('btn-back').addEventListener('click', () => {
  if (currentView === 'study' || currentView === 'review' || currentView === 'deck') showView(currentView === 'deck' ? 'decks' : 'deck');
  else showView('decks');
});
